...
def get_news():
    ...
    ticker = resolve_ticker(symbol)
    # Alpaca usa simboli senza suffissi: 'GC=F' → 'GC', 'AAPL' → 'AAPL'
    alpaca_symbol = ticker.split("=")[0].split("-")[0].replace("^", "")
            ...
            try:
                ...
                for article in ddgs_news:
                    try:
                        ...
                        logger.warning(f"[NEWS API] Errore parsing data notizie DDG: {e_date}")
            except Exception as search_e:
                logger.warning(f"[NEWS API] DDGS API fallito ({search_e}), uso fallback web HTML...")
...
def get_calibrazione():
    ...
    try:
        ...
        return jsonify({
            ...
            "TECH_SHORT_TERM_CANDLES":  Calibrazione.TECH_SHORT_TERM_CANDLES,
            "TECH_MID_TERM_CANDLES":    Calibrazione.TECH_MID_TERM_CANDLES,
            "TECH_LONG_TERM_CANDLES":   Calibrazione.TECH_LONG_TERM_CANDLES,
