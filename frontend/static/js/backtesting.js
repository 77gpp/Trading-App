...
function buildAgentAccordions() {
  ...
  // 1. Costruiamo i 3 accordion dinamici con il placeholder
  const dynamicGroups = ['pattern', 'trend', 'sr'];
  const dynamicMeta   = {
...
function buildDynamicAccordions(chosenTools) {
  ...
  // Rimuoviamo tutti i checkbox/overlay esistenti dei 3 gruppi dinamici
  ...
  ['pattern', 'trend', 'sr'].forEach(key => {
    ...
    // FILTRO: Teniamo solo quelli con segnali reali sul grafico
  ...
  const totalActive   = activePattern + activeTrend + activeSR;
  ...
  if (checkbox.checked) {
    ...
    if (toolId === 'volume_profile') {
      ...
      if (hasData) {
        ...
        // NOVITÀ: Niente più toast di errore fastidioso.
        // Se l'utente clicca manualmente e non c'è dato, diamo un info soft.
...
function getCalibrazione() {
  ...
  const keys = [
    ...
    'ALPACA_NEWS_LIMIT', 'DUCKDUCKGO_NEWS_LIMIT', 'TECH_SHORT_TERM_CANDLES',
    'TECH_MID_TERM_CANDLES', 'TECH_LONG_TERM_CANDLES', 'AGENT_MACRO_ENABLED',
...
async function loadRealAfterData(symbol, endDate) {
  try {
    ...
    if (realEnd > new Date()) {
      realEnd.setDate(new Date().getDate());
    }
