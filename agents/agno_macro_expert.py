...
class AgnoMacroExpert:
    ...
    def __init__(self):
        ...
        self.agent = Agent(
            ...
            description=(
                ...
                "Usa il tool get_alpaca_news per le ultime notizie ufficiali per lo specifico simbolo. "
            ...
            instructions=[
                ...
                "Utilizza il tool get_alpaca_news per ottenere le notizie ufficiali di mercato per lo specifico simbolo nel periodo richiesto.",
    ...
    def analizza(self, query="Analizza l'attuale scenario globale", start_date=None, end_date=None, symbol=None):
        ...
        # ── 1. Pre-fetch Alpaca News (date esatte garantite) ─────────────
        alpaca_section = ""
        if start_date:
            logger.info(f"[AGNO MACRO] Pre-fetch Alpaca news: {symbol} | {start_date} → {end_date or 'oggi'}")
            alpaca_raw = get_alpaca_news(symbol, start=start_date, end=end_date)
            alpaca_section = (
                f"\n\n---\nNOTIZIE ALPACA MARKETS ({start_date} → {end_date or 'oggi'}) — già recuperate con date esatte:\n"
                f"{alpaca_raw}\n"
                f"[NON chiamare di nuovo il tool get_alpaca_news: i dati sopra sono già completi e nel periodo corretto]\n---\n"
            )
        ...
        enriched_query = f"""PERIODO DI ANALISI: {start_date or 'non specificato'} → {end_date or 'oggi'}
{duckduckgo_instruction}
{alpaca_section}{yfinance_section}
QUERY: {query}"""
    ...
    def sintetizza_verdetto(self, nome_asset: str, macro_sentiment: str, results_tech: dict) -> str:
        """
...
Ora consulta la tua skill 'trading-verdict-synthesizer' e produci il VERDETTO FINALE operativo.
Applica il decision framework professionale: verifica i no-trade filters, calcola il confluence score,
e poi decidi Bias, Entry, Stop Loss e Target usando i livelli di prezzo esatti identificati dagli specialisti.
IMPORTANTE: Rispondi ESCLUSIVAMENTE in italiano. Inizia direttamente con '**Bias Primario**:' (oppure con '**NO TRADE**' se i filtri lo impongono).
"""
        logger.info(f"[AGNO MACRO] Generazione verdetto finale per {nome_asset}...")
